...
There are several different approaches for loading external data into a Frontend component.

It's possible for Frontend components to load their own data internally on the client, or on the server using [React server components](/docs/integrating-editor/server-components). This doesn't require any Frontend configuration.
