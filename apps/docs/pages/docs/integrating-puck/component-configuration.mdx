import { ConfigPreview, EditorPreview } from "@/docs/components/Preview";
import { Editor } from "@/editor";

# Component Configuration

Frontend's core behaviour is configured via the [Config](/docs/api-reference/configuration/config). This describes:

- which components are available to Frontend
...
The [Config](/docs/api-reference/configuration/config) is provided via the `config` prop to the main Frontend components:

- [`<Editor>`](/docs/api-reference/components/editor) reads the Config and renders an editor UI. The user interacts with the editor to produce a [data payload](/docs/api-reference/data-model/data).
...
This tells Frontend that **HeadingBlock** is a valid component, and describes how to render it.

When the user drags the component onto the preview and hits **Publish** in the editor UI via the `<Editor>` component, this Config will produce a [data payload](/docs/api-reference/data-model/data) like this:
...
The data payload and Config together tell `<Render>` how to render the page. It can also be provided to `<Editor>` as an [initial `data` payload](/docs/api-reference/components/editor#data).
...
Unlike [default parameters](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Default_parameters), `defaultProps` are stored in the data payload and will populate the Frontend fields.
