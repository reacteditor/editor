...
React Editor's core behaviour is configured via the [Config](/docs/api-reference/configuration/config). This describes:

- which components are available to React Editor
...
The [Config](/docs/api-reference/configuration/config) is provided via the `config` prop to the main React Editor components:
...
This tells React Editor that **HeadingBlock** is a valid component, and describes how to render it.
...
Unlike [default parameters](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Default_parameters), `defaultProps` are stored in the data payload and will populate the React Editor fields.
