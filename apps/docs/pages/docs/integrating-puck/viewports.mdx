...
Puck provides 4 viewports [by default](/docs/api-reference/components/editor#default-viewports):
...
When implementing a [compositional interface](/docs/extending-editor/composition), the `viewports` API will have no effect. Instead, the viewport size can be controlled by the dimensions of the wrapping element that contains [`<Puck.Preview />`](/docs/api-reference/components/editor-preview).
