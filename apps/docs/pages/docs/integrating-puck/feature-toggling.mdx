...
Feature toggling is enabled by Frontend's [Permissions API](/docs/api-reference/permissions). This enables you to toggle behavior like:
...
Toggling features across the entire Frontend instance can be done with global permissions. These can be set by the [`permissions` prop](/docs/api-reference/components/editor#permissions) on the Frontend component:
