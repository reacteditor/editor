...
Frontend supports nested and multi-column layouts across any CSS layout using the [`slot` field](/docs/api-reference/fields/slot).
...
By default, Frontend will wrap your components in a `div` element. For some layouts, you may need to eliminate the wrapping element and treat the child component as a direct descendant of its' parent slot.
...
const config = {
  components: {
    ...
    Card: {
      inline: true, // Enable inline mode, removing the Frontend wrapper
      render: ({ text, spanCol, spanRow, editor }) => (
        <div
          ref={editor.dragRef} // Let Frontend know this element is draggable
          style={{
        ...
        render: ({ content, spanCol, spanRow, editor }) => {
          return (
            <div
              ...
              // Let Frontend know this element is draggable
              ref={editor.dragRef}
            >
              {content}
            </div>
