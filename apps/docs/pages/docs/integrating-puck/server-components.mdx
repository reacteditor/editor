...
Frontend provides support for [React Server Components](https://react.dev/reference/react/use-server#use-server) (RSC), but the interactive-nature of Frontend requires special consideration.

## Environments

### Server

Frontend supports the server environment for the following APIs:

- The [`<Render>`](/docs/api-reference/components/render) component, for rendering pages produced by Frontend
- The [`resolveAllData`](/docs/api-reference/functions/resolve-all-data) lib, for running all [data resolvers](/docs/integrating-editor/dynamic-props)

These APIs can be used in an RSC environment, but in order to do so the Frontend config that they reference must be RSC-friendly.
...
All other Frontend APIs, including the core `<Editor>` component, cannot run in an RSC environment due to their high-degree of interactivity.

As these APIs render on the client, the Frontend config provided must be safe for client-use, avoiding any server-specific logic.

## Implementations

Since the Frontend config can be referenced on the client or the server, we need to consider how to satisfy both environments.
...
```tsx copy showLineNumbers filename="editor.config.tsx"
...
import { config } from "../editor.config.tsx";
...
Alternatively, consider entirely separate configs for the `<Editor>` and `<Render>` components. This approach can enable you to have different rendering behavior for a component for when it renders on the client or the server.

Create a shared config type:

```tsx copy showLineNumbers filename="editor-types.ts"
import type { Config } from "@frontend/core";
...
Define a client component config for use within the `<Editor>` component:

```tsx copy showLineNumbers filename="editor.config.client.tsx"
import type { UserConfig } from "./editor-types.ts";
...
```tsx copy showLineNumbers filename="editor.config.server.tsx"
import type { UserConfig } from "./editor-types.ts";
...
```tsx copy showLineNumbers filename="app/page.tsx"
import { config } from "../editor.config.server.tsx";
...
If your server config omits those render-relevant fields entirely, Frontend cannot infer which props should be transformed before render. In practice this means rich text will be escaped as plain strings, and slot data will not be converted into nested components.
