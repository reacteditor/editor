...
React Editor provides support for [React Server Components](https://react.dev/reference/react/use-server#use-server) (RSC), but the interactive-nature of React Editor requires special consideration.

## Environments

### Server

React Editor supports the server environment for the following APIs:

- The [`<Render>`](/docs/api-reference/components/render) component, for rendering pages produced by React Editor
...
These APIs can be used in an RSC environment, but in order to do so the React Editor config that they reference must be RSC-friendly.
...
All other React Editor APIs, including the core `<Editor>` component, cannot run in an RSC environment due to their high-degree of interactivity.

As these APIs render on the client, the React Editor config provided must be safe for client-use, avoiding any server-specific logic.

## Implementations

Since the React Editor config can be referenced on the client or the server, we need to consider how to satisfy both environments.
...
If your server config omits those render-relevant fields entirely, React Editor cannot infer which props should be transformed before render. In practice this means rich text will be escaped as plain strings, and slot data will not be converted into nested components.
