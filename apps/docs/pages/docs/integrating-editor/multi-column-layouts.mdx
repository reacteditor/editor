...
React Editor supports nested and multi-column layouts across any CSS layout using the [`slot` field](/docs/api-reference/fields/slot).

<Callout type="info">Slots replace the [`<DropZone>` component](/docs/api-reference/components/drop-zone) component, which will soon be deprecated and removed.</Callout>
...
By default, React Editor will wrap your components in a `div` element. For some layouts, you may need to eliminate the wrapping element and treat the child component as a direct descendant of its' parent slot.

For example, this is required if you wish to use CSS rules like `flex-grow`, `grid-column`, or `grid-row`.

Use the [`inline`](/docs/api-reference/configuration/component-config#inline) component parameter to remove the wrapping element. **When using this API, you must also specify which element is draggable by passing the [`editor.dragRef` prop](/docs/api-reference/configuration/component-config#editordragref) to your element's `ref` prop.**
...
const config = {
  components: {
    ...
    Card: {
      inline: true, // Enable inline mode, removing the React Editor wrapper
      render: ({ text, spanCol, spanRow, editor }) => (
        <div
          ref={editor.dragRef} // Let React Editor know this element is draggable
          style={{
...
<EditorPreview
  ...
  config={{
    ...
    components: {
      Card: {
        inline: true,
        render: ({ content, spanCol, spanRow, editor }) => {
          return (
            <div
              style={{
                background: "white",
              ...
              // Let React Editor know this element is draggable
              ref={editor.dragRef}
            >
              {content}
            </div>
...
Combine this with [categories](/docs/integrating-editor/categories) to restrict behavior based on your existing groups.
