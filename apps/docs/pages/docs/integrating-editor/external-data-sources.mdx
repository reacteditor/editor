...
There are several different approaches for loading external data into a React Editor component.

It's possible for React Editor components to load their own data internally on the client, or on the server using [React server components](/docs/integrating-editor/server-components). This doesn't require any React Editor configuration.
...
- [`contentful`](https://github.com/editoreditor/editor/tree/main/packages/field-contentful): Select content entries from a [Contentful](https://www.contentful.com) space.
