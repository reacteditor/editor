...
The internal state of the [`<Editor>`](/docs/api-reference/components/editor) component.

## `data`

The current [`Data`](/docs/api-reference/data-model/data) payload being managed by React Editor.

## `ui`

The current state of the React Editor editor interface.
...
React Editor does not currently provide the UI to control this value, but it can be toggled via the `cmd+i` or `ctrl+i` hotkeys.
...
| [`options`](#uiviewportsoptions)                 | `[]`                              | [Viewports\[\]](/docs/api-reference/components/editor#viewport-params) |
...
The available viewport options, as provided via the [`viewports` API](/docs/api-reference/components/editor#viewports).
