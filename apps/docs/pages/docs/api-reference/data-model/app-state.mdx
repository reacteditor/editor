...
The current [`Data`](/docs/api-reference/data-model/data) payload being managed by Frontend.

## `ui`

The current state of the Frontend editor interface.
...
Frontend does not currently provide the UI to control this value, but it can be toggled via the `cmd+i` or `ctrl+i` hotkeys.
