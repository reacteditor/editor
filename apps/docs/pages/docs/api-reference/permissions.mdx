...
Permissions enable the [toggling of React Editor features](/docs/integrating-editor/feature-toggling). There are three types of permissions:

1. **Global permissions** are shared across the entire React Editor application. Controlled via the `permissions` prop on the `<Editor>` component.
...
1. Global [`permissions` prop](/docs/api-reference/components/editor#permissions)
...
Custom permissions enable you to control your own functionality when [extending React Editor](/docs/extending-editor/composition).
