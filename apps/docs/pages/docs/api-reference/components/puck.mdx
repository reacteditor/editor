...
Sets the undo/redo Puck history state when using the `usePuck` [history API](/docs/api-reference/editor-api#history).
...
Callback that triggers when Puck dispatches an [action](https://editoreditor.com/docs/api-reference/actions), like `insert` or `set`. Use this to track changes, perform side effects, or sync with external systems.
