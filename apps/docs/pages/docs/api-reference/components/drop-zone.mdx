...
<Callout>
The [`<DropZone>` component](/docs/api-reference/components/drop-zone) component is being replaced by the [`slot` field](/docs/api-reference/fields/slot), and will soon be deprecated and removed.
</Callout>

Place droppable regions (zones) inside other components to enable nested components.

```tsx {1,9} copy
import { DropZone } from "@reacteditor/core";
...
Configure which axis React Editor will use for overlap collision detection.
...
Instead, you can use the [`renderDropZone` method](/docs/api-reference/configuration/component-config#propseditorrenderdropzone) passed to your component render function.
