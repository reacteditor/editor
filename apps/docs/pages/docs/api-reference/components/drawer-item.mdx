...
<EditorPreview config={{}} data={{ root: { props: {} }, content: [] }}>
  <Drawer>
    <Drawer.Item name="Orange">{() => <div>Orange 🍊</div>}</Drawer.Item>
  </Drawer>
</EditorPreview>
...
If using the `<Drawer>` as a component list to be dragged into `<Editor.Preview>`, this should be the key of a component defined in the [Config](/docs/api-reference/configuration/config).
