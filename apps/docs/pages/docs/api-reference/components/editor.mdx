...
| [`routes`](#routes)                                               | `routes: ["/", "/about"]`                          | String\[\]                                                               | -            |
| [`currentRoute`](#currentroute)                                   | `currentRoute: "/about"`                           | String                                                                   | -            |
| [`onRouteChange()`](#onroutechangepath)                           | `onRouteChange: (path) => {}`                      | Function                                                                 | -            |
...
Sets the undo/redo React Editor history state when using the `useEditor` [history API](/docs/api-reference/editor-api#history).
...
Append the React Editor [`data`](#data) prop onto the end of [`histories`](#histories). Defaults to `true`.
...
Callback that triggers when React Editor dispatches an [action](https://editoreditor.com/docs/api-reference/actions), like `insert` or `set`. Use this to track changes, perform side effects, or sync with external systems.
...
An [`Overrides`](/docs/api-reference/overrides) object defining custom render methods for various parts of the React Editor UI.
...
Set the global [permissions](/docs/api-reference/permissions) for the React Editor instance to toggle React Editor functionality.
...
An array of plugins to enhance React Editor's behaviour. See the [Plugin API reference](/docs/api-reference/plugin).

The [`blocksPlugin`](/docs/api-reference/plugins/blocks-plugin) (Components panel) is **auto-injected** by the editor shell. The [`outlinePlugin`](/docs/api-reference/plugins/outline-plugin) (Outline panel) is **not** — pass it in the array to enable that panel. The fields panel is also auto-added unless you supply a plugin named `"fields"`.
...
Pair with [`onRouteChange`](#onroutechangepath) to control navigation. Most apps don't set this directly — [`<App>`](/docs/api-reference/components/app) wires it up automatically based on the keys in its `pages` map.
...
The route key currently being edited. Used together with [`routes`](#routes) to highlight the active entry in the editor's page switcher.
...
Configure the viewports available to the user, rendered as an iframe. React Editor will select the most appropriate initial viewport based on the user's window size, unless otherwise specified via the [`ui`](#ui) prop.
...
React Editor uses [Lucide icons](https://lucide.dev/icons/). You can use [lucide-react](https://lucide.dev/guide/packages/lucide-react) to choose a similar icon, if desired.
