...
A hook for accessing the [`PuckApi`](/docs/api-reference/editor-api) as part of your React render lifecycle.. The best way to access `usePuck` is via the `createUsePuck()` factory.
...
A selector function that describes what `usePuck` returns. Receives [`PuckApi`](/docs/api-reference/editor-api) and returns anything. Be as granular as possible to minimize re-renders.
