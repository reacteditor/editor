...
- [`emotion-cache`](https://github.com/editoreditor/editor/tree/main/packages/plugin-emotion-cache): Inject emotion cache into the Puck iframe.
...
To load a plugin, provide it to the [`plugins` prop](/docs/api-reference/components/editor#plugins) on the `<Puck>` component.
...
You can leverage the [internal Puck API](internal-editor-api) to integrate Puck behavior with your plugin:
...
Plugins support [Field Transforms](/docs/extending-editor/field-transforms), enabling you to modify prop data before it's rendered in the `<Puck>` preview.
...
Plugins support [UI Overrides](/docs/extending-editor/ui-overrides), enabling you to override discrete section of the Puck interface.
...
import { registerOverlayPortal } from "@editoreditor/core";
