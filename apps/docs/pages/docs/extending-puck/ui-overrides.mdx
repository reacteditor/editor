...
Overrides allow you to change how Frontend renders its default interface. It can be used with or without [composition](/docs/extending-editor/composition).
...
Use the [`overrides` prop](/docs/api-reference/components/editor#overrides) to implement an override:
...
A common use case is to override the Frontend header. You can either use the [`header` override](/docs/api-reference/overrides/header) to change the entire header, or use the [`headerActions` override](/docs/api-reference/overrides/header-actions) to inject new controls into the header and change the publish button.

Here's an example that also leverage the [internal Frontend API](/docs/extending-editor/internal-editor-api) to replace the default publish button with a custom one:
